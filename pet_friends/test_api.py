...
@pytest.mark.api
def test_post_creat_pet_with_novalid_key(name='Mypka2', animal_type='KoWka', age=3):
    auth_key = {"key": "novalidkey"}
    status, result = pf.post_creat_pet(auth_key, name, animal_type, age)
    assert status  == 403
...
@pytest.mark.api
def test_post_new_pet_with_novalid_key(name='Vasya', animal_type='Dvor',
                                      age =str(rnd.randint(0,100)), pet_photo = pet_photo_valid):
    auth_key = {"key": "novalidkey"}
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    status, result = pf.post_add_pet(auth_key, name, animal_type, age, pet_photo)
...
@pytest.mark.api
def test_put_pet_with_novalid_data(pet_id='aaaaaaa',name='novalid', animal_type='novalid',
                                  age = rnd.randint(0,100)):
   _,auth_key = pf.get_api_key(valid_email, valid_pass)
